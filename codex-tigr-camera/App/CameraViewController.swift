...
final class CameraViewController: UIViewController {
    ...
    private func buildInterface() {
        ...
        profileLabel.text = settings.profile.rawValue
    ...
    @objc private func toggleVideoMode() {
        ...
        profileLabel.text = isVideoMode ? videoSettings.profile.rawValue : settings.profile.rawValue
        sessionQueue.async { [weak self] in
            guard let self else { return }
            if self.isVideoMode { self.applyVideoConfiguration(showFallback: true) }
            else {
    ...
    @objc private func modeChanged() {
        ...
        videoInfoLabel.isHidden = true; storageLabel.isHidden = true; profileLabel.text = settings.profile.rawValue
    ...
    private func processCapturedFrames(_ frames: [CapturedFrame]) {
        ...
        processingQueue.async { [weak self] in
            ...
            let context = PhotoProcessingContext(scene: analysis.scene, brightness: analysis.brightness, dynamicRange: analysis.dynamicRange, iso: deviceISO, flash: usedFlash, stable: stable)
            let result = PhotoProcessor.process(frames.map(\.image), mode: mode, settings: prefs, context: context)
            DispatchQueue.main.async {
                guard let self else { return }
                guard let image = result, let data = ImageEncoder.jpeg(image, metadata: frames.first?.metadata ?? [:], quality: 0.96) else { self.finishCapture(message: "Ошибка обработки"); return }
                ...
                self.finishCapture(message: "Фото сохранено • \(analysis.scene.rawValue)")
            }
        }
    }
    ...
    @objc private func openSettings() {
        ...
        controller.onSave = { [weak self] newSettings, newVideoSettings, exposure in
            ...
            self.profileLabel.text = self.isVideoMode ? self.videoSettings.profile.rawValue : self.settings.profile.rawValue
...
private enum PhotoProcessor {
    ...
    private static func tone(_ image: CIImage, mode: PhotoMode, settings s: ProcessingSettings, processingContext: PhotoProcessingContext) -> CIImage {
        ...
        let luminanceNoise = min(0.085, 0.008 + s.denoise * 0.065 + max(0, processingContext.iso - 400) / 18_000)
        ...
        result = result.applyingFilter("CIHighlightShadowAdjust", parameters: [
            "inputShadowAmount": min(0.78, s.shadows + modeBoost),
            "inputHighlightAmount": max(0.20, 1 - s.highlights * (s.preserveSky ? 0.86 : 0.62))
        ])
        result = result.applyingFilter("CIColorControls", parameters: [kCIInputSaturationKey: saturation, kCIInputContrastKey: contrast, kCIInputBrightnessKey: night ? -0.008 : 0])
        result = result.applyingFilter("CIVibrance", parameters: ["inputAmount": s.profile == .forestGolden ? 0.12 : 0.045])
        ...
        let tint: CGFloat = (processingContext.scene == .indoor || processingContext.scene == .screen) ? -2.0 : 0
        ...
        result = result.applyingFilter("CIUnsharpMask", parameters: [kCIInputRadiusKey: 1.25, kCIInputIntensityKey: min(0.34, s.sharpness * 0.42)])
        result = result.applyingFilter("CISharpenLuminance", parameters: [kCIInputSharpnessKey: 0.08 + s.sharpness * 0.58])
